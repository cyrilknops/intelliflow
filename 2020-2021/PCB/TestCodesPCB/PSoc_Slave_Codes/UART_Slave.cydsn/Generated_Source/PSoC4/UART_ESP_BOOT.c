...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_ESP) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_ESP_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_ESP_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_ESP_GetRxBufferSize();
                CyDelay(UART_ESP_BYTE2BYTE_TIME_OUT);
            }
            while(UART_ESP_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_ESP_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_ESP_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_ESP_GetRxBufferSize() > 0u)
    {
        tempCount = UART_ESP_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_ESP_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_ESP_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_ESP_PACKET_EOP)
            {
                CyDelay(UART_ESP_WAIT_EOP_DELAY);
            }
        }
