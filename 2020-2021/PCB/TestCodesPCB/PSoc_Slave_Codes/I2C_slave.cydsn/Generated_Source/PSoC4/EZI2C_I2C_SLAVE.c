...
volatile uint8 EZI2C_slStatus;          /* Slave Status */

/* Receive buffer variables */
volatile uint8 * EZI2C_slWrBufPtr;      /* Pointer to Receive buffer  */
volatile uint32  EZI2C_slWrBufSize;     /* Slave Receive buffer size  */
volatile uint32  EZI2C_slWrBufIndex;    /* Slave Receive buffer Index */

/* Transmit buffer variables */
volatile uint8 * EZI2C_slRdBufPtr;      /* Pointer to Transmit buffer  */
volatile uint32  EZI2C_slRdBufSize;     /* Slave Transmit buffer size  */
volatile uint32  EZI2C_slRdBufIndex;    /* Slave Transmit buffer Index */
volatile uint32  EZI2C_slRdBufIndexTmp; /* Slave Transmit buffer Index Tmp */
volatile uint8   EZI2C_slOverFlowCount; /* Slave Transmit Overflow counter */

#if (EZI2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)

    uint32 (*EZI2C_customAddressHandler)(void) = NULL;

    /*******************************************************************************
    * Function Name: EZI2C_SetI2cAddressCustomInterruptHandler
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void EZI2C_SetI2cAddressCustomInterruptHandler(uint32 (*func) (void))
    {
        ...
        EZI2C_customAddressHandler = func;
    }
#endif /* (EZI2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */


/*******************************************************************************
* Function Name: EZI2C_I2CSlaveStatus
****************************************************************************//**
*
...
*   - EZI2C_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - EZI2C_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    EZI2C_I2C_SSTAT_RD_CMPLT is set.
*   - EZI2C_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - EZI2C_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - EZI2C_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - EZI2C_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    EZI2C_I2C_SSTAT_WR_CMPLT is set.
*   - EZI2C_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - EZI2C_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     EZI2C_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
...
{
    return((uint32) EZI2C_slStatus);
}


/*******************************************************************************
* Function Name: EZI2C_I2CSlaveClearReadStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear EZI2C_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  EZI2C_slStatus  - used to store current status of I2C slave.
*
...
uint32 EZI2C_I2CSlaveClearReadStatus(void)
{
    ...
    status = ((uint32) EZI2C_slStatus & EZI2C_I2C_SSTAT_RD_MASK);
    EZI2C_slStatus &= (uint8) ~EZI2C_I2C_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: EZI2C_I2CSlaveClearWriteStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear EZI2C_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  EZI2C_slStatus  - used to store current status of I2C slave.
*
...
uint32 EZI2C_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = ((uint32) EZI2C_slStatus & EZI2C_I2C_SSTAT_WR_MASK);
    EZI2C_slStatus &= (uint8) ~EZI2C_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: EZI2C_I2CSlaveSetAddress
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: EZI2C_I2CSlaveSetAddressMask
****************************************************************************//**
*
...
void EZI2C_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~EZI2C_RX_MATCH_MASK_MASK);
...
*
* \globalvars
*  EZI2C_slRdBufPtr   - used to store pointer to slave read buffer.
*  EZI2C_slRdBufSize  - used to store salve read buffer size.
*  EZI2C_slRdBufIndex - used to store current index within slave
*  read buffer.
*
*******************************************************************************/
void EZI2C_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        EZI2C_slRdBufPtr      = (volatile uint8 *) rdBuf; /* Set buffer pointer  */
        EZI2C_slRdBufSize     = bufSize;                  /* Set buffer size     */
        EZI2C_slRdBufIndex    = 0u;                       /* Clear buffer index  */
        EZI2C_slRdBufIndexTmp = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: EZI2C_I2CSlaveInitWriteBuf
****************************************************************************//**
*
*  Sets the buffer pointer and size of the write buffer. This function also 
*  resets the transfer count returned with the 
*  EZI2C_I2CSlaveGetWriteBufSize() function.
*
...
*
* \globalvars
*  EZI2C_slWrBufPtr   - used to store pointer to slave write buffer.
*  EZI2C_slWrBufSize  - used to store salve write buffer size.
...
void EZI2C_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        EZI2C_slWrBufPtr   = (volatile uint8 *) wrBuf; /* Set buffer pointer  */
        EZI2C_slWrBufSize  = bufSize;                  /* Set buffer size     */
        EZI2C_slWrBufIndex = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: EZI2C_I2CSlaveGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes read by the I2C master since an
*  EZI2C_I2CSlaveInitReadBuf() or 
*  EZI2C_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if EZI2C_I2C_SSTAT_RD_ERR was 
...
{
    return(EZI2C_slRdBufIndex);
}


/*******************************************************************************
* Function Name: EZI2C_I2CSlaveGetWriteBufSize
****************************************************************************//**
*
*  Returns the number of bytes written by the I2C master since an
*  EZI2C_I2CSlaveInitWriteBuf() or 
*  EZI2C_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if EZI2C_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  EZI2C_slWrBufIndex - used to store current index within slave
...
{
    return(EZI2C_slWrBufIndex);
}


/*******************************************************************************
* Function Name: EZI2C_I2CSlaveClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: EZI2C_I2CSlaveClearWriteBuf
****************************************************************************//**
*
