...
{
    CyRamVectors[CYINT_IRQ_BASE + RX_interrupt__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + RX_interrupt__INTC_NUMBER];
}
...
void RX_interrupt_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((RX_interrupt__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *RX_interrupt_INTC_PRIOR = (*RX_interrupt_INTC_PRIOR & (uint32)(~RX_interrupt__INTC_PRIOR_MASK)) |
...
uint8 RX_interrupt_GetPriority(void)
{
    ...
    priority = (*RX_interrupt_INTC_PRIOR & RX_interrupt__INTC_PRIOR_MASK) >> priorityOffset;
